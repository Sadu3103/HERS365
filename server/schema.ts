...
export const teams = pgTable('teams', {
  ...
  hasQuestionnaire: boolean('has_questionnaire').default(false),
  applicationUrl: text('application_url'),
  questionnaireUrl: text('questionnaire_url'),
  socials: jsonb('socials'), // { instagram, twitter, facebook, youtube }
...
export const nilActivities = pgTable('nil_activities', {
  id: serial('id').primaryKey(),
  playerId: integer('player_id').references(() => players.id),
...
export const nilOpportunities = pgTable('nil_opportunities', {
  id: serial('id').primaryKey(),
...
export const dealApplications = pgTable('deal_applications', {
  id: serial('id').primaryKey(),
  playerId: integer('player_id').references(() => players.id),
...
export const earningsTracking = pgTable('earnings_tracking', {
  id: serial('id').primaryKey(),
  playerId: integer('player_id').references(() => players.id),
  applicationId: integer('application_id').references(() => dealApplications.id),
...
export const teamNilCampaigns = pgTable('team_nil_campaigns', {
  id: serial('id').primaryKey(),
...
export const mentorshipConnections = pgTable('mentorship_connections', {
  ...
  matchedAt: timestamp('matched_at').default(sql`now()`),
});
...
export const playerHighlights = pgTable('player_highlights', {
  ...
  clipSettings: jsonb('clip_settings'), // { start, end, originalVideoId }
  createdAt: timestamp('created_at').default(sql`now()`),
});
...
export const skillChallengeCompletions = pgTable('skill_challenge_completions', {
  id: serial('id').primaryKey(),
  playerId: integer('player_id').references(() => players.id),
...
export const eventLeaderboards = pgTable('event_leaderboards', {
  id: serial('id').primaryKey(),
  eventId: integer('event_id').references(() => events.id),
  playerId: integer('player_id').references(() => players.id),
  rank: integer('rank'),
...
export const athleteRankings = pgTable('athlete_rankings', {
  id: serial('id').primaryKey(),
  ...
  combineScore: doublePrecision('combine_score'),
...
export const savedScholarships = pgTable('saved_scholarships', {
  ...
  savedAt: text('saved_at').default(sql`now()`),
});
export const faqs = pgTable('faqs', {
  ...
  lastAskedAt: text('last_asked_at').default(sql`now()`),
  createdAt: timestamp('created_at').default(sql`now()`),
});
...
export const eventStore = pgTable('event_store', {
  ...
  deadLetter: boolean('dead_letter').default(false),
...
export const sagaInstances = pgTable('saga_instances', {
  ...
  compensating: boolean('compensating').default(false),
  retryCount: integer('retry_count').default(0),
  maxRetries: integer('max_retries').default(3),
  errorMessage: text('error_message'),
...
export const serviceHealth = pgTable('service_health', {
  ...
  lastHealthCheck: text('last_health_check').default(sql`now()`),
...
export const serviceRegistry = pgTable('service_registry', {
  ...
  lastHeartbeat: text('last_heartbeat').default(sql`now()`),
...
export const distributedLocks = pgTable('distributed_locks', {
  ...
  acquiredAt: text('acquired_at').default(sql`now()`),
...
export const commandStore = pgTable('command_store', {
  id: text('id').primaryKey(),
...
// ── Recruiting feature tables (extends the existing `teams` table) ───────────
...
export const programDetails = pgTable('program_details', {
  ...
  minGpa: text('min_gpa'),
  rosterNeeds: jsonb('roster_needs'),
  athleticBenchmarks: jsonb('athletic_benchmarks'),
  eligibilityNotes: text('eligibility_notes'),
  majorsList: jsonb('majors_list'),
