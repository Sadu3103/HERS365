 ...
 *
 * Usage: npx tsx server/scripts/seed-programs.ts
 */
...
async function seed() {
  ...
  for (const school of SCHOOLS) {
    ...
    const existing = await db
      ...
      .from(schema.teams)
      .where(eq(schema.teams.name, school.name))
      .limit(1);
    ...
    const existingDetail = await db
      .select({ id: schema.programDetails.id })
    ...
    if (existingDetail.length === 0) {
      await db.insert(schema.programDetails).values({
        ...
        hasScholarships: school.hasScholarships,
      });
    }
  }
