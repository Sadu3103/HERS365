...
export const Messages = () => {
  ...
  const DEMO_CONVERSATIONS: Conversation[] = [
    { partnerId: 101, partnerName: 'Coach Marcus Vance', partnerRole: 'Head Coach • Stanford', lastMessage: 'Looking forward to seeing you at our elite showcase next month!', lastMessageAt: new Date(Date.now() - 3600000).toISOString(), unreadCount: 1 },
    { partnerId: 102, partnerName: 'Coach Elena Rodriguez', partnerRole: 'Recruiting Coordinator • UCLA', lastMessage: 'We reviewed your highlight tape—outstanding footwork and poise.', lastMessageAt: new Date(Date.now() - 86400000).toISOString(), unreadCount: 0 }
  ];
  const DEMO_THREADS: Record<number, ThreadMessage[]> = {
    101: [
      { id: 1, content: 'Coach Marcus Vance: Looking forward to seeing you at our elite showcase next month!', isFromMe: false, read: true, createdAt: new Date(Date.now() - 3600000).toISOString() },
    ],
    102: [
      { id: 2, content: 'Coach Elena Rodriguez: We reviewed your highlight tape—outstanding footwork and poise.', isFromMe: false, read: true, createdAt: new Date(Date.now() - 86400000).toISOString() },
    ]
  };
  ...
  const allConversations = (convData?.data && convData.data.length > 0) ? convData.data : DEMO_CONVERSATIONS;
  ...
  const thread = (threadData?.data && threadData.data.length > 0) ? threadData.data : (activePartner != null ? (DEMO_THREADS[activePartner] || DEMO_THREADS[101]) : []);
