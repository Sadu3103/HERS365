  ...
  const toggleDiamondOverride = async (userId: number, currentOverride?: boolean | null) => {
    ...
    try {
      const res = await fetch(`/api/admin/data/users/${userId}/toggle-diamond`, {
        ...
        body: JSON.stringify({ diamondOverride: nextVal }),
      });
      const data = await res.json();
      if (data.success) {
        setSignups(prev =>
          prev.map(s => (s.id === userId ? { ...s, diamondOverride: nextVal } : s))
        );
      }
    } catch (err) {
      console.error('Failed to toggle diamond features', err);
    }
  };
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: `1px solid ${LINE}` }}>
                  {['Name', 'Position', 'State', 'Diamond Access Status', 'Admin VIP Toggle', 'Joined'].map((h) => (
                    <th
                      key={h}
                      style={{
                        textAlign: 'left',
              ...
              <tbody>
                {signups.map((s, i) => {
                  const hasStripeSub = s.subscriptionTier === 'pro' || s.subscriptionTier === 'college' || s.subscriptionTier === 'high_school' || s.subscriptionTier === 'elite';
                  const hasOverride = Boolean(s.diamondOverride);
                  return (
                    <tr
                      key={s.id}
                      style={{
                    ...
                    >
                      ...
                      <td style={{ padding: '10px 16px' }}>
                        <button
                          onClick={() => toggleDiamondOverride(s.id, s.diamondOverride)}
                          style={{
                            background: hasOverride ? '#FF2E93' : 'rgba(255,255,255,0.08)',
                            border: hasOverride ? '1px solid #FF2E93' : '1px solid rgba(255,255,255,0.2)',
                            color: hasOverride ? '#fff' : '#f4f4f2',
                            padding: '5px 12px',
                            borderRadius: 8,
                            fontSize: '0.72rem',
                            fontWeight: 800,
                        ...
                        >
                          <Shield size={12} />
                          {hasOverride ? 'Override: ACTIVE' : 'Grant Diamond VIP'}
                        </button>
                      </td>
