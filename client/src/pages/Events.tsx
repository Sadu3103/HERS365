...
type EventItem = {
  ...
  title: string;
  org: string;
  type: 'Tournament' | 'Camp' | '7v7' | 'Combine' | 'Showcase';
  date: string;
  city: string;
  state: string;
  cost: string;
...
const EVENTS: EventItem[] = [
  { id: 1, title: 'HERS365 Spring Showcase', org: 'HERS365', type: 'Showcase', date: 'Jun 28, 2026', city: 'Los Angeles', state: 'CA', cost: 'Free', spots: 64, spotsLeft: 18, registered: false, featured: true, desc: 'The official HERS365 recruiting showcase. College scouts invited. Compete against top athletes in Southern California.' },
  { id: 2, title: 'NorCal 7v7 Summer Classic', org: 'Bay Area Flag', type: '7v7', date: 'Jul 12, 2026', city: 'San Jose', state: 'CA', cost: '$45/athlete', spots: 128, spotsLeft: 44, registered: false, featured: false, desc: '16-team bracket. Two-day tournament with film sessions after each round. Coaching staff on site.' },
  { id: 3, title: 'Elite QB-WR Camp', org: 'Routes Academy', type: 'Camp', date: 'Jul 19–20, 2026', city: 'Austin', state: 'TX', cost: '$125', spots: 40, spotsLeft: 9, registered: true, featured: false, desc: 'Two-day intensive for QBs and pass catchers. Film review, route trees, 1-on-1s, and college prep sessions.' },
  { id: 4, title: 'NFHS Girls Flag Combine', org: 'NFHS', type: 'Combine', date: 'Aug 2, 2026', city: 'Atlanta', state: 'GA', cost: '$60', spots: 100, spotsLeft: 31, registered: false, featured: true, desc: 'Official NFHS combine. 40-yard dash, shuttle, vertical, position drills. Results distributed to participating programs.' },
  { id: 5, title: 'Southwest Regional Tournament', org: 'USA Flag Football', type: 'Tournament', date: 'Aug 16–17, 2026', city: 'Phoenix', state: 'AZ', cost: '$200/team', spots: null, spotsLeft: null, registered: false, featured: false, desc: 'Regional qualifier for the USA Flag Football National Championship. Open to U17 and U19 divisions.' },
  { id: 6, title: 'Florida All-Star Showcase', org: 'Florida Girls Flag', type: 'Showcase', date: 'Sep 6, 2026', city: 'Miami', state: 'FL', cost: '$35', spots: 80, spotsLeft: 52, registered: false, featured: false, desc: 'All-star format with east vs west roster draft. College coaches from 4 programs confirmed.' },
];
...
export const Events = () => {
  ...
  const register = (id: number) => {
    setEvents((prev) => prev.map((e) => e.id === id ? { ...e, registered: !e.registered, spotsLeft: e.spotsLeft !== null ? e.spotsLeft + (e.registered ? 1 : -1) : null } : e));
  };
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '28px 20px 120px' }}>
      ...
      <AnimatePresence>
        {filtered.map((ev) => <EventCard key={ev.id} ev={ev} onRegister={() => register(ev.id)} />)}
      </AnimatePresence>
    </div>
  );
};
