    ...
    <div className="min-h-screen bg-dark-900 flex items-center justify-center p-6 cyber-grid">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {!isLogin && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input type="text" placeholder="Athletic Name" value={name} onChange={e => setName(e.target.value)} className="w-full bg-dark-900/50 border border-white/5 rounded-2xl py-4 pl-12 pr-4 focus:outline-none focus:border-brand-500/50 transition-all text-white placeholder:text-dark-600" />
              </div>
            </motion.div>
          )}
          <div className="space-y-2">
            ...
            <div className="relative group">
              ...
              <input type="email" placeholder="you@example.com" value={email} onChange={e => setEmail(e.target.value)} required className="w-full bg-dark-900/50 border border-white/5 rounded-2xl py-4 pl-12 pr-4 focus:outline-none focus:border-brand-500/50 transition-all text-white placeholder:text-dark-600" />
            </div>
          </div>
          <div className="space-y-2">
            ...
            <div className="relative group">
              ...
              <input type="password" placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} required className="w-full bg-dark-900/50 border border-white/5 rounded-2xl py-4 pl-12 pr-4 focus:outline-none focus:border-brand-500/50 transition-all text-white placeholder:text-dark-600" />
            </div>
          </div>
          {error && <p className="text-red-400 text-sm font-semibold text-center">{error}</p>}
          <button type="submit" disabled={loading} className="w-full py-5 bg-brand-500 hover:bg-brand-600 disabled:opacity-60 disabled:cursor-not-allowed text-white rounded-2xl font-black uppercase tracking-[0.2em] transition-all shadow-xl shadow-brand-500/30 flex items-center justify-center gap-3 group relative overflow-hidden">
        ...
        <p className="text-center mt-12 text-sm text-dark-400 font-medium">
          ...
          <button onClick={() => { setIsLogin(!isLogin); setError(''); }} className="ml-2 text-brand-500 font-black uppercase tracking-widest hover:text-brand-400 transition-colors underline-offset-4 hover:underline">
