...
import { Mail, Lock, User, ArrowRight, Eye, EyeOff, Chrome, Github, ArrowUpRight } from 'lucide-react';
      ...
      <div style={{ position: 'relative' }}>
        ...
        <input
          ...
          style={{
            ...
            fontSize: '0.95rem', color: TEXT, fontFamily: BODY,
        ...
        {isPass && (
          <button
            ...
            style={{
              ...
              padding: 6, display: 'flex', alignItems: 'center', transition: 'color .2s',
              borderRadius: 8,
            }}
...
export const Auth = () => {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: INK, color: TEXT, fontFamily: BODY, overflowX: 'hidden' }}>
      ...
      >
        ...
        />
        {/* one restrained warm wash, low and left */}
        <div
          aria-hidden
          style={{
            position: 'absolute', width: 620, height: 620, borderRadius: '50%',
            filter: 'blur(120px)', opacity: 0.16, bottom: -280, left: -180,
            background: `radial-gradient(circle, ${FLAME}, transparent 62%)`, pointerEvents: 'none',
          }}
        />
        ...
        >
          ...
          >
            <span style={{ width: 6, height: 6, borderRadius: '50%', background: FLAME, boxShadow: `0 0 10px ${FLAME}` }} />
            Girls Flag Football
          </div>
      ...
      {/* ── RIGHT PANEL (form) ── */}
      <main style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 24px', position: 'relative', background: INK }}>
        <motion.div
        ...
        >
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <h2 style={{ fontFamily: DISP, fontWeight: 900, fontSize: 'clamp(2.4rem,5vw,3rem)', textTransform: 'uppercase', lineHeight: 0.92, margin: '0 0 10px', letterSpacing: '.01em' }}>
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            <Field id="auth-email" label="Email Address" type="email" icon={Mail} value={email} onChange={setEmail} required autoComplete="email" />
            <Field id="auth-password" label="Password" type="password" icon={Lock} value={password} onChange={setPassword} required autoComplete={isLogin ? 'current-password' : 'new-password'} />

            {isLogin && (
              <div style={{ textAlign: 'right', marginTop: 2, marginBottom: 20 }}>
                <button
                  type="button"
                  onClick={() => navigate('/forgot-password')}
                  style={{ background: 'none', border: 'none', color: MUTED, fontSize: '.72rem', fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', cursor: 'pointer', fontFamily: DISP, padding: '2px 0', transition: 'color .2s' }}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {[
              ...
              <button
                ...
                onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.06)'; e.currentTarget.style.borderColor = LINE_2; e.currentTarget.style.color = TEXT; }}
          ...
          >
            {isLogin ? (
              ...
              <>
                ...
                <Link to="/terms" style={{ color: MUTED, textDecoration: 'underline', textDecorationColor: 'rgba(154,154,150,0.4)' }}>Terms</Link>{' '}
                and{' '}
                <Link to="/privacy" style={{ color: MUTED, textDecoration: 'underline', textDecorationColor: 'rgba(154,154,150,0.4)' }}>Privacy Policy</Link>
              </>
            )}
          </p>
        </motion.div>
