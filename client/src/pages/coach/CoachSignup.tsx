    ...
    <div className="min-h-screen bg-surface flex items-center justify-center p-6">
      ...
      >
        <div className="flex flex-col items-center mb-10">
          ...
          <p className="text-ink-muted font-medium tracking-wide text-center">Create your account to discover and recruit athletes.</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            <label className="text-xs font-black uppercase tracking-[0.2em] text-ink-muted ml-1">Name</label>
          ...
          <div className="space-y-2">
            <label className="text-xs font-black uppercase tracking-[0.2em] text-ink-muted ml-1">Confirm Password</label>
            <div className="relative group">
              <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-ink-muted group-focus-within:text-coral-500 transition-colors" size={20} />
              <input
                type="password" placeholder="••••••••"
          ...
          <div className="space-y-2">
            <label className="text-xs font-black uppercase tracking-[0.2em] text-ink-muted ml-1">School / Organization</label>
            <div className="relative group">
              <Building2 className="absolute left-4 top-1/2 -translate-y-1/2 text-ink-muted group-focus-within:text-coral-500 transition-colors" size={20} />
              <input
          ...
          <div className="space-y-2">
            <label className="text-xs font-black uppercase tracking-[0.2em] text-ink-muted ml-1">Sport</label>
            <div className="relative group">
              <Trophy className="absolute left-4 top-1/2 -translate-y-1/2 text-ink-muted group-focus-within:text-coral-500 transition-colors" size={20} />
              <input
          ...
          <div className="space-y-2">
            <label className="text-xs font-black uppercase tracking-[0.2em] text-ink-muted ml-1">Division</label>
            <div className="relative group">
              <Shield className="absolute left-4 top-1/2 -translate-y-1/2 text-ink-muted group-focus-within:text-coral-500 transition-colors z-10" size={20} />
              <select
                value={division} onChange={e => setDivision(e.target.value)} required
                className="w-full bg-surface/50 border border-white/5 rounded-2xl py-4 pl-12 pr-4 focus:outline-none focus:border-coral-500/50 transition-all text-white appearance-none cursor-pointer"
              >
                {DIVISIONS.map(d => (
                  <option key={d} value={d} className="bg-surface text-white">{d}</option>
                ))}
              </select>
              <ChevronRight className="absolute right-4 top-1/2 -translate-y-1/2 text-ink-muted rotate-90 pointer-events-none" size={18} />
            </div>
          </div>
        ...
        <p className="text-center mt-8 text-sm text-ink-muted">
          ...
          <Link to="/coach/login" className="text-coral-500 font-black uppercase tracking-widest hover:text-coral-400 transition-colors">
            Sign In
          </Link>
        </p>
      </motion.div>
