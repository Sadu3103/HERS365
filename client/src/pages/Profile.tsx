...
import {
  ...
  Activity, X, Award, Star, Shield, Target, Flame, Medal, LogOut
} from 'lucide-react';
...
const DEMO_PROFILE: ApiProfile = {
  ...
  gradYear: 2026,
  gpa: '3.9',
  bio: 'Elite Dual-Threat QB • #1 Flag Football Recruit in CA • 2x All-State • Stanford Commit',
  achievements: '2025 National MVP, All-American 1st Team, State Champion',
  verificationStatus: 'verified',
  g5Rating: 98.4,
  archetype: 'Field General / Elite Dual-Threat',
  nilPoints: 4250,
  heightIn: 68,
  weightLbs: 142,
  profileImage: null,
...
const DEMO_GAME_STATS: GameStat[] = [
  {
    passingAttempts: 28,
...
const DEMO_COMBINE_STATS: CombineStat = {
  fortyDash: '4.58s',
  shuttle: '4.12s',
  vertical: '32"',
  broadJump: '9\'8"',
...
export const Profile = () => {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      ...
      try {
        ...
        if (!data || !data.name) { setProfile(DEMO_PROFILE); } else { setProfile(data); }
      } catch {
  ...
  useEffect(() => {
    ...
    if (isOwnProfile) {
      apiFetch<{ game: GameStat[]; combine: CombineStat | null }>('/api/profile/stats')
        .then(d => {
          setGameStats(Array.isArray(d?.game) && d.game.length > 0 ? d.game : DEMO_GAME_STATS);
          setCombineStats(d?.combine ?? DEMO_COMBINE_STATS);
        })
        .catch(() => {
          setGameStats(DEMO_GAME_STATS);
          setCombineStats(DEMO_COMBINE_STATS);
        })
        .finally(() => setStatsLoading(false));
    } else {
      apiFetch<GameStat[]>(`/api/players/${profile.id}/stats`)
        .then(d => { setGameStats(Array.isArray(d) && d.length > 0 ? d : DEMO_GAME_STATS); })
        .catch(() => {
          setGameStats(DEMO_GAME_STATS);
        })
        .finally(() => setStatsLoading(false));
  ...
  useEffect(() => {
    ...
    const demoHl: Highlight[] = [
      {
        id: 1,
        videoUrl: 'https://images.unsplash.com/photo-1541534741688-6078c6bfb5c5?w=800&q=80',
        thumbnailUrl: 'https://images.unsplash.com/photo-1541534741688-6078c6bfb5c5?w=800&q=80',
        category: 'Game Reel',
        season: '2026',
    ...
    apiFetch<Highlight[]>(`/api/players/${profile.id}/highlights`)
      .then(d => setHighlights(Array.isArray(d) && d.length > 0 ? d : demoHl))
      .catch(() => setHighlights(demoHl))
  ...
  useEffect(() => {
    ...
    const demoBadges: Badge[] = [
      { id: 1, name: 'Verified Athlete', description: 'Official stats verified against MaxPreps', icon: 'check', category: 'verification', earnedAt: '2026-01-15' },
      { id: 2, name: 'All-State 1st Team', description: 'Selected for 2025 All-State First Team', icon: 'award', category: 'honor', earnedAt: '2026-02-01' }
    ];
    apiFetch<{ success: boolean; data: Badge[] }>(`/api/badges/${profile.id}`)
      .then(d => setBadges(Array.isArray(d?.data) && d.data.length > 0 ? d.data : demoBadges))
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 4 }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 4 }}>
                  <h1 style={{ fontFamily: 'Barlow Condensed, sans-serif', fontWeight: 800, fontSize: '1.6rem', textTransform: 'uppercase', color: '#fff', lineHeight: 1, margin: 0 }}>{profile.name}</h1>
                  {verified && (
                    <span
                      style={{
                        display: 'inline-flex',
                      ...
                      title="Reviewed and verified by HERS365 Moderation against official reference"
                    >
                      🟢 Official Stats Verified
                    </span>
                  )}
                </div>
              ...
              >
                ...
                >
                  💎 {((profile.g5Rating ?? 88) / 20).toFixed(1)} Diamonds
                </div>
              </div>
            </div>
        ...
        return (
          <div className="k-card" style={{ padding: '16px 20px', marginBottom: 20 }}>
            ...
            <div style={{ height: 6, borderRadius: 9999, background: 'rgba(255,255,255,0.06)', overflow: 'hidden', marginBottom: 14 }}>
              <div style={{ width: `${pct}%`, height: '100%', background: 'linear-gradient(90deg,#8B3BFF,#FF2E93)', transition: 'width .4s' }} />
            </div>
          ...
          <div style={{ marginTop: 28, paddingBottom: 24 }}>
            <button
              ...
              style={{
                ...
                color: '#ff453a', fontWeight: 800, fontSize: '0.88rem', letterSpacing: '0.06em',
